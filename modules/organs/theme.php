...
function detail_per($data_content)
{
    ...
    if (!empty($global_organ_rows[$data_content['organid']])) {
        $data_content['ofogran'] =
            $global_organ_rows[$data_content['organid']]['title'];
        $data_content['ofogran_url'] =
            NV_BASE_SITEURL .
            'index.php?' .
            NV_LANG_VARIABLE .
            '=' .
            ...
            NV_OP_VARIABLE .
            '=vieworg/' .
            $global_organ_rows[$data_content['organid']]['alias'] .
            '-' .
            $data_content['organid'];
    }
    ...
    if (defined('NV_IS_MODADMIN')) {
        $admin_link =
            nv_link_edit_per($data_content['personid']) .
            ' ' .
            nv_link_delete_per($data_content['personid']);
    }
...
function vieworg_list($organs_data, $person_data, $html_pages)
{
    ...
    if (defined('NV_IS_MODADMIN')) {
        $admin_link =
            nv_link_edit_org($organs_data['organid']) .
            ' ' .
            nv_link_delete_org($organs_data['organid']);
    }
    $xtpl->assign('admin_link', $admin_link);
            ...
            foreach ($array_content as $content) {
                ...
                if (!empty($content['data'])) {
                    ...
                    foreach ($content['data'] as $person) {
                        $suborganid = [];
                        if (
                            $person['organid'] != $org_item and
                            empty($array_op[1]) and
                            count([$suborganid]) > 1
                        ) {
                        ...
                        if (!empty($person['position_other'])) {
                            $person['position_other'] =
                                '</br>' . $person['position_other'];
                        }
                        if (!empty($person['professional'])) {
                            $person['professional'] =
                                '</br>' . $person['professional'];
                        }
                        $xtpl->assign('ROW', $person);
                        ...
                        if (
                            !empty($person['mobile']) or
                            !empty($person['phone']) or
                            !empty($person['phone_ext'])
                        ) {
