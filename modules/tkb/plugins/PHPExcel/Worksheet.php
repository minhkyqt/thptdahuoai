...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    public function __construct(PHPExcel $pParent = null, $pTitle = 'Worksheet')
    {
        ...
        $this->cellCollection = PHPExcel_CachedObjectStorageFactory::getInstance(
            $this
        );
        // Set page setup
        ...
        // Default column dimension
        $this->defaultColumnDimension = new PHPExcel_Worksheet_ColumnDimension(
            null
        );
    ...
    public function refreshColumnDimensions()
    {
        ...
        foreach ($currentColumnDimensions as $objColumnDimension) {
            $newColumnDimensions[
                $objColumnDimension->getColumnIndex()
            ] = $objColumnDimension;
        }
    ...
    public function calculateColumnWidths($calculateMergeCells = false)
    {
        ...
        if (!empty($autoSizes)) {
            ...
            foreach ($this->getMergeCells() as $cells) {
                foreach (
                    PHPExcel_Cell::extractAllCellReferencesInRange($cells)
                    as $cellReference
                ) {
            ...
            foreach ($this->getCellCollection(false) as $cellID) {
                ...
                if (
                    ...
                    if (
                        !isset(
                            $isMergeCell[
                                $this->cellCollection->getCurrentAddress()
                            ]
                        )
                    ) {
                        // Calculated value
                        ...
                        $autoSizes[
                            $this->cellCollection->getCurrentColumn()
                        ] = max(
                            (float) $autoSizes[
                                $this->cellCollection->getCurrentColumn()
                            ],
                ...
                while ($this->parent->sheetNameExists($pValue . ' ' . $i)) {
                    ...
                    if ($i == 10) {
                        if (
                            ...
                            $pValue = PHPExcel_Shared_String::Substring(
                                $pValue,
                                0,
                                28
                            );
    ...
    public function setCellValueByColumnAndRow(
        ...
        $cell = $this->getCellByColumnAndRow($pColumn, $pRow)->setValue(
            $pValue
        );
        return $returnCell ? $cell : $this;
    }
    ...
    public function setCellValueExplicit(
        ...
        // Set value
        $cell = $this->getCell(strtoupper($pCoordinate))->setValueExplicit(
            $pValue,
            $pDataType
        );
        return $returnCell ? $cell : $this;
    ...
    public function setCellValueExplicitByColumnAndRow(
        ...
        $cell = $this->getCellByColumnAndRow($pColumn, $pRow)->setValueExplicit(
            $pValue,
            $pDataType
        );
        return $returnCell ? $cell : $this;
    ...
    public function getCell($pCoordinate = 'A1', $createIfNotExists = true)
    {
        ...
        if (strpos($pCoordinate, '!') !== false) {
            $worksheetReference = PHPExcel_Worksheet::extractSheetTitle(
                $pCoordinate,
                true
            );
            return $this->parent
        ...
        if (
            !preg_match(
                '/^' . PHPExcel_Calculation::CALCULATION_REGEXP_CELLREF . '$/i',
                $pCoordinate,
                $matches
            ) &&
            preg_match(
                '/^' .
                    PHPExcel_Calculation::CALCULATION_REGEXP_NAMEDRANGE .
                    '$/i',
                $pCoordinate,
                $matches
            )
        ) {
            $namedRange = PHPExcel_NamedRange::resolveRange(
                $pCoordinate,
                $this
            );
            if ($namedRange !== null) {
                ...
                return $namedRange
                    ->getWorksheet()
                    ->getCell($pCoordinate, $createIfNotExists);
            }
        }
    ...
    private function createNewCell($pCoordinate)
    {
        ...
        if (
            PHPExcel_Cell::columnIndexFromString($this->cachedHighestColumn) <
            PHPExcel_Cell::columnIndexFromString($aCoordinates[0])
        ) {
            $this->cachedHighestColumn = $aCoordinates[0];
        }
        $this->cachedHighestRow = max(
    ...
    {
        return $this->cellExists(
            PHPExcel_Cell::stringFromColumnIndex($pColumn) . $pRow
        );
    }
    ...
    public function getRowDimension($pRow = 1, $create = true)
    {
        ...
        if (!isset($this->rowDimensions[$pRow])) {
            ...
            $this->rowDimensions[$pRow] = new PHPExcel_Worksheet_RowDimension(
                $pRow
            );
    ...
    public function getColumnDimension($pColumn = 'A', $create = true)
    {
        ...
        if (!isset($this->columnDimensions[$pColumn])) {
            ...
            $this->columnDimensions[
                $pColumn
            ] = new PHPExcel_Worksheet_ColumnDimension($pColumn);
     ...
     * @return PHPExcel_Style
     */
    public function getStyleByColumnAndRow(
        $pColumn = 0,
        $pRow = 1,
        $pColumn2 = null,
        $pRow2 = null
     ...
     * @return PHPExcel_Worksheet
     */
    public function setSharedStyle(
        PHPExcel_Style $pSharedCellStyle = null,
        $pRange = ''
    ) {
    ...
    public function duplicateStyle(
        PHPExcel_Style $pCellStyle = null,
        $pRange = ''
    ) {
        // make sure we have a real style and not supervisor
        $style = $pCellStyle->getIsSupervisor()
            ? $pCellStyle->getSharedComponent()
            : $pCellStyle;

        // Add the style to the workbook if necessary
        $workbook = $this->parent;
        if (
            $existingStyle = $this->parent->getCellXfByHashCode(
                $pCellStyle->getHashCode()
            )
        ) {
        ...
        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
            for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row) {
                $this->getCell(
                    PHPExcel_Cell::stringFromColumnIndex($col - 1) . $row
                )->setXfIndex($xfIndex);
            }
        }
     ...
     * @return PHPExcel_Worksheet
     */
    public function duplicateConditionalStyle(
        array $pCellStyle = null,
        $pRange = ''
    ) {
     ...
     * @return PHPExcel_Worksheet
     */
    public function duplicateStyleArray(
        $pStyles = null,
        $pRange = '',
        $pAdvanced = true
    ) {
     ...
     * @return PHPExcel_Worksheet
     */
    public function setBreakByColumnAndRow(
        $pColumn = 0,
        $pRow = 1,
        $pBreak = PHPExcel_Worksheet::BREAK_NONE
    ) {
    ...
    public function mergeCells($pRange = 'A1:A1')
    {
        ...
        if (strpos($pRange, ':') !== false) {
            ...
            if (!$this->cellExists($upperLeft)) {
                $this->getCell($upperLeft)->setValueExplicit(
                    null,
            ...
            for ($i = 1; $i < $count; $i++) {
                if ($this->cellExists($aReferences[$i])) {
                    $this->getCell($aReferences[$i])->setValueExplicit(
                        null,
                        PHPExcel_Cell_DataType::TYPE_NULL
                    );
                }
     ...
     * @return PHPExcel_Worksheet
     */
    public function mergeCellsByColumnAndRow(
        $pColumn1 = 0,
        $pRow1 = 1,
        $pColumn2 = 0,
        $pRow2 = 1
    ...
    public function unmergeCells($pRange = 'A1:A1')
    {
        ...
        if (strpos($pRange, ':') !== false) {
            if (isset($this->mergeCells[$pRange])) {
                ...
                throw new PHPExcel_Exception(
                    'Cell range ' . $pRange . ' not known as merged.'
                );
            }
        } else {
            throw new PHPExcel_Exception(
     ...
     * @return PHPExcel_Worksheet
     */
    public function unmergeCellsByColumnAndRow(
        $pColumn1 = 0,
        $pRow1 = 1,
        $pColumn2 = 0,
        $pRow2 = 1
    ...
    public function protectCells(
        ...
        if (!$pAlreadyHashed) {
            $pPassword = PHPExcel_Shared_PasswordHasher::hashPassword(
                $pPassword
            );
        }
    ...
    public function unprotectCells($pRange = 'A1')
    {
        ...
        if (isset($this->protectedCells[$pRange])) {
            ...
            throw new PHPExcel_Exception(
                'Cell range ' . $pRange . ' not known as protected.'
            );
        }
        return $this;
    }
    ...
    public function setAutoFilter($pValue)
    {
        ...
        if (is_string($pValue)) {
            ...
            is_object($pValue) &&
            $pValue instanceof PHPExcel_Worksheet_AutoFilter
        ) {
     ...
     *    @return PHPExcel_Worksheet
     */
    public function setAutoFilterByColumnAndRow(
        $pColumn1 = 0,
        $pRow1 = 1,
        $pColumn2 = 0,
        $pRow2 = 1
    ...
    public function insertNewRowBefore($pBefore = 1, $pNumRows = 1)
    {
        if ($pBefore >= 1) {
            ...
            $objReferenceHelper->insertNewBefore(
                'A' . $pBefore,
                0,
                $pNumRows,
                $this
    ...
    {
        if ($pBefore >= 0) {
            return $this->insertNewColumnBefore(
                PHPExcel_Cell::stringFromColumnIndex($pBefore),
                $pNumCols
            );
        } else {
            throw new PHPExcel_Exception(
    ...
    public function removeColumn($pColumn = 'A', $pNumCols = 1)
    {
        if (!is_numeric($pColumn)) {
            ...
            $pColumn = PHPExcel_Cell::stringFromColumnIndex(
                PHPExcel_Cell::columnIndexFromString($pColumn) - 1 + $pNumCols
            );
            $objReferenceHelper = PHPExcel_ReferenceHelper::getInstance();
            $objReferenceHelper->insertNewBefore(
                $pColumn . '1',
            ...
            for ($c = 0; $c < $pNumCols; ++$c) {
                ...
                $highestColumn = PHPExcel_Cell::stringFromColumnIndex(
                    PHPExcel_Cell::columnIndexFromString($highestColumn) - 2
                );
            }
        } else {
            throw new PHPExcel_Exception(
            ...
            for ($col = $minCol; $col != $maxCol; ++$col) {
                ...
                if ($this->cellCollection->isDataSet($col . $row)) {
                    ...
                    if ($cell->getValue() !== null) {
                        ...
                        if ($formatData) {
                            $style = $this->parent->getCellXfByIndex(
                                $cell->getXfIndex()
                            );
                            $returnValue[$rRef][
                                $cRef
    ...
    public function namedRangeToArray(
        ...
        throw new PHPExcel_Exception(
            'Named Range ' . $pNamedRange . ' does not exist.'
        );
    }
    ...
    {
        return new PHPExcel_Worksheet_ColumnIterator(
            $this,
            $startColumn,
            $endColumn
        );
    ...
    public function garbageCollect()
    {
        ...
        $highestColumn = PHPExcel_Cell::columnIndexFromString(
            $colRow['column']
        );
        ...
        if ($highestColumn < 0) {
            ...
            $this->cachedHighestColumn = PHPExcel_Cell::stringFromColumnIndex(
                --$highestColumn
            );
        }
    ...
    public function getDataValidation($pCellCoordinate = 'A1')
    {
        ...
        $this->dataValidationCollection[
            $pCellCoordinate
        ] = new PHPExcel_Cell_DataValidation();
        return $this->dataValidationCollection[$pCellCoordinate];
    }
    ...
    public function shrinkRangeToFit($range)
    {
        ...
        foreach ($rangeBlocks as &$rangeSet) {
            ...
            if (
                PHPExcel_Cell::columnIndexFromString($rangeBoundaries[0][0]) >
                $maxCol
            ) {
                $rangeBoundaries[0][0] = PHPExcel_Cell::stringFromColumnIndex(
                    $maxCol
                );
            }
            ...
            if (
                PHPExcel_Cell::columnIndexFromString($rangeBoundaries[1][0]) >
                $maxCol
            ) {
                $rangeBoundaries[1][0] = PHPExcel_Cell::stringFromColumnIndex(
                    $maxCol
                );
            }
        ...
        foreach ($this as $key => $val) {
            ...
            if (is_object($val) || is_array($val)) {
                if ($key == 'cellCollection') {
                    ...
                    $key == 'autoFilter' &&
                    $this->autoFilter instanceof PHPExcel_Worksheet_AutoFilter
                ) {
    ...
    public function setCodeName($pValue = null)
    {
        ...
        if ($this->getParent()) {
            ...
            if ($this->getParent()->sheetCodeNameExists($pValue)) {
                ...
                $i = 1;
                while (
                    $this->getParent()->sheetCodeNameExists($pValue . '_' . $i)
                ) {
                    ++$i;
                    if ($i == 10) {
                        if (
