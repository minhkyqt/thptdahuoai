...
function nv_comment_data($module, $area, $id, $page, $sortcomm, $base_url)
{
    ...
    $comment_array = [];
    $per_page_comment = empty($module_config[$module]['perpagecomm'])
        ? 5
        : $module_config[$module]['perpagecomm'];
    ...
    if ($num_items) {
        ...
        $db_slave
            ->select(
                'a.cid, a.pid, a.content, a.attach, a.post_time, a.post_name, a.post_email, a.likes, a.dislikes, b.userid, b.username, b.email, b.first_name, b.last_name, b.photo, b.view_mail'
            )
        ...
        while ($row = $result->fetch()) {
            ...
            if (!empty($row['attach'])) {
                $row['attach'] =
                    ...
                    $row['cid'] .
                    '&amp;tokend=' .
                    md5($row['cid'] . '_' . NV_CHECK_SESSION);
            }
            $comment_array[$row['cid']] = $row;
        }
        if (!empty($comment_list_id)) {
            foreach ($comment_list_id as $cid) {
                $comment_array[$cid]['subcomment'] = nv_comment_get_reply(
                    $cid,
                    $module,
                    NV_CHECK_SESSION,
                    $sortcomm
            ...
            $result->closeCursor();
            unset($row, $result);
            $generate_page = nv_generate_page(
                $base_url,
                $num_items,
                $per_page_comment,
                $page,
...
function nv_comment_get_reply($cid, $module, $session_id, $sortcomm)
{
    ...
    if ($num_items_sub) {
        $emailcomm = $module_config[$module]['emailcomm'];
        $db_slave->select(
            'a.cid, a.pid, a.content, a.attach, a.post_time, a.post_name, a.post_email, a.likes, a.dislikes, b.userid, b.email, b.first_name, b.last_name, b.photo, b.view_mail'
        );
...
function nv_comment_load(
    ...
    if (!empty($module) and isset($module_config[$module]['activecomm'])) {
        if (
            $id > 0 and
            $module_config[$module]['activecomm'] == 1 and
            $checkss ==
                md5(
                    $module .
                        '-' .
            ...
            $view_comm = nv_user_in_groups(
                $module_config[$module]['view_comm']
            );
            if ($view_comm) {
                $allowed_comm = nv_user_in_groups($allowed);
                $sortcomm_old = $nv_Request->get_int(
                    'sortcomm',
                    'cookie',
                    $module_config[$module]['sortcomm']
                );
                $sortcomm = $nv_Request->get_int(
                    'sortcomm',
                    'post',
                    $sortcomm_old
                );
                ...
                $base_url =
                    ...
                    '&checkss=' .
                    $checkss .
                    '&comment_load=1&perpage=' .
                    $per_page_comment;
                $comment_array = nv_comment_data(
                    $module,
                    $area,
                    $id,
                    $page,
                ...
                if (defined('NV_IS_SPADMIN')) {
                    ...
                    global $admin_info;
                    $adminscomm = array_map(
                        'intval',
                        explode(',', $module_config[$module]['adminscomm'])
                    );
                    if (
                        in_array(
                            (int) $admin_info['admin_id'],
                            $adminscomm,
                            true
                        )
                    ) {
        ...
        if (
            ...
            if (
                ...
                if (isset($allowed_tmp['4']) or isset($allowed_tmp['7'])) {
                    ...
                    $list_groups = array_intersect_key(
                        nv_groups_list_pub(),
                        $allowed_tmp
                    );
            ...
            if (defined('NV_IS_SPADMIN')) {
                ...
                if (
                    in_array((int) $admin_info['admin_id'], $adminscomm, true)
                ) {
                    $is_delete = true;
                }
            }
            if ($view_comm) {
                ...
                $comment = nv_comment_module_data(
                    $module,
                    $comment_array,
                    $is_delete,
                    $allowed_comm,
            ...
            return nv_theme_comment_module(
                $module,
                $area,
                $id,
                $allowed,
 ...
 * @return string
 */
function nv_theme_comment_module(
    $module,
    $area,
    $id,
    $allowed_comm,
    ...
    $template = file_exists(
        NV_ROOTDIR .
            ...
            '/modules/comment/main.tpl'
    )
        ? $global_config['module_theme']
        : 'default';
    $templateCSS = file_exists(
        NV_ROOTDIR .
            '/themes/' .
            $global_config['module_theme'] .
            '/css/comment.css'
    ...
    $xtpl = new XTemplate(
        'main.tpl',
        NV_ROOTDIR . '/themes/' . $template . '/modules/comment'
    );
    ...
    if ($allowed_comm) {
        ...
        $captcha = (int) $module_config[$module];
        $show_captcha = true;
        if ($captcha == 0) {
            $show_captcha = false;
        ...
        $captcha_type =
            (empty($module_config['comment']['captcha_type']) or
            in_array(
                $module_config['comment']['captcha_type'],
                ['captcha', 'recaptcha'],
                true
            ))
                ? $module_config['comment']['captcha_type']
                : 'captcha';
        if (
            $captcha_type == 'recaptcha' and
            (empty($global_config['recaptcha_sitekey']) or
                empty($global_config['recaptcha_secretkey']))
        ) {
        ...
        if ($show_captcha) {
            if (
                ...
                $xtpl->assign(
                    'CAPTCHA_REFR_SRC',
                    NV_STATIC_URL . NV_ASSETS_DIR . '/images/refresh.png'
                );
                $xtpl->assign(
                    'SRC_CAPTCHA',
                    NV_BASE_SITEURL .
                        'index.php?scaptcha=captcha&t=' .
                        NV_CURRENTTIME
                );
        ...
        if ($form_login['mode'] == 'direct') {
            ...
            $xtpl->assign(
                'LOGIN_MESSAGE',
                sprintf(
                    $lang_module_comment['comment_login'],
                    $form_login['groups'][0]
                )
            );
            ...
            $xtpl->assign(
                'LANG_REG_GROUPS',
                sprintf(
                    $lang_module_comment['comment_register_groups'],
                    implode(', ', $form_login['groups']),
                    $form_login['link']
                )
            );
 ...
 * @return string
 */
function nv_comment_module_data(
    $module,
    $comment_array,
    $is_delete,
    $allowed_comm,
    ...
    if (!empty($comment_array['comment'])) {
        ...
        foreach ($comment_array['comment'] as $comment_array_i) {
            if (!empty($comment_array_i['subcomment'])) {
                $comment_array_reply = nv_comment_module_data_reply(
                    $module,
                    $comment_array_i['subcomment'],
                    $is_delete,
                    $allowed_comm
                );
            ...
            $comment_array_i['post_time'] = nv_date(
                'd/m/Y H:i',
                $comment_array_i['post_time']
            );

            if (
                !empty($comment_array_i['photo']) and
                file_exists(NV_ROOTDIR . '/' . $comment_array_i['photo'])
            ) {
                $comment_array_i['photo'] =
                    NV_BASE_SITEURL . $comment_array_i['photo'];
            } elseif (
                is_file(
                    NV_ROOTDIR .
                        '/themes/' .
                        $global_config['module_theme'] .
                        '/images/users/no_avatar.png'
                )
            ) {
                $comment_array_i['photo'] =
                    NV_STATIC_URL .
                ...
                $comment_array_i['photo'] =
                    NV_STATIC_URL . 'themes/default/images/users/no_avatar.png';
            }

            if (!empty($comment_array_i['userid'])) {
                $comment_array_i['post_name'] = nv_show_name_user(
                    $comment_array_i['first_name'],
                    $comment_array_i['last_name'],
                    $comment_array_i['username']
                );
            }
            ...
            if (
                $module_config[$module]['emailcomm'] and
                !empty($comment_array_i['post_email'])
            ) {
 ...
 * @return string
 */
function nv_comment_module_data_reply(
    $module,
    $comment_array,
    $is_delete,
    $allowed_comm
