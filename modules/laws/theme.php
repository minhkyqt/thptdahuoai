...
function nv_theme_laws_list(
    ...
    foreach ($array_data as $row) {
        $row['url_subject'] =
            ...
            '=subject/' .
            $row['alias'];
        $row['publtime'] = $row['publtime']
            ? nv_date('d/m/Y', $row['publtime'])
            : 'N/A';
        $row['exptime'] = $row['exptime']
            ? nv_date('d/m/Y', $row['exptime'])
            : 'N/A';
        $row['number_comm'] = $row['number_comm']
            ? sprintf(
                $lang_module['number_comm'],
                number_format($row['number_comm'], 0, ',', '.')
            )
            : '';
        ...
        if ($module_config[$module_name]['activecomm']) {
            ...
            if (!empty($row['start_comm_time'])) {
                $comment_time[] = sprintf(
                    $lang_module['start_comm_time'],
                    nv_date('d/m/Y', $row['start_comm_time'])
                );
            }
            if (!empty($row['end_comm_time'])) {
                $comment_time[] = sprintf(
                    $lang_module['end_comm_time'],
                    nv_date('d/m/Y', $row['end_comm_time'])
                );
            }
...
function nv_theme_laws_maincat($mod, $array_data)
{
    ...
    foreach ($array_data as $data) {
        $data['url_subject'] =
            ...
            '=subject/' .
            $data['alias'];
        $data['numcount'] = sprintf(
            $lang_module['s_result_num'],
            $data['numcount']
        );
        ...
        if (!empty($data['rows'])) {
            $xtpl->assign(
                'HTML',
                nv_theme_laws_list($data['rows'], '', false, false)
            );
...
function nv_theme_laws_detail(
    ...
    $xtpl = new XTemplate(
        $module_info['funcs'][$op]['func_name'] . '.tpl',
        NV_ROOTDIR .
            '/themes/' .
            $module_info['template'] .
            '/modules/' .
    ...
    $array_data['publtime'] = $array_data['publtime']
        ? nv_date('d/m/Y', $array_data['publtime'])
        : '';
    $array_data['startvalid'] = $array_data['startvalid']
        ? nv_date('d/m/Y', $array_data['startvalid'])
        : '';
    $array_data['exptime'] = $array_data['exptime']
        ? nv_date('d/m/Y', $array_data['exptime'])
        : '';
    $array_data['start_comm_time'] = $array_data['start_comm_time']
        ? nv_date('d/m/Y', $array_data['start_comm_time'])
        : $lang_module['unlimit'];
    $array_data['end_comm_time'] = $array_data['end_comm_time']
        ? nv_date('d/m/Y', $array_data['end_comm_time'])
        : $lang_module['unlimit'];
    $array_data['approval'] =
        $array_data['approval'] == 1 ? $lang_module['e1'] : $lang_module['e0'];
    if (isset($nv_laws_listcat[$array_data['cid']])) {
        ...
        $array_data['cat_url'] =
            ...
            NV_OP_VARIABLE .
            '=' .
            $nv_laws_listcat[$array_data['cid']]['alias'];
    } else {
        $array_data['cat'] = '';
        $array_data['cat_url'] = '#';
    }
    ...
    if (isset($nv_laws_listsubject[$array_data['sid']])) {
        $array_data['subject'] =
            $nv_laws_listsubject[$array_data['sid']]['title'];
        $array_data['subject_url'] =
            ...
            NV_OP_VARIABLE .
            '=subject/' .
            $nv_laws_listsubject[$array_data['sid']]['alias'];
    } else {
        $array_data['subject'] = '';
        $array_data['subject_url'] = '';
    }
    ...
    $filled_field = 0;
    if (
        empty($nv_laws_setting['detail_hide_empty_field']) or
        !empty($array_data['cat'])
    ) {
        $filled_field++;
    ...
    if (
        (empty($nv_laws_setting['detail_hide_empty_field']) or
            !empty($array_data['publtime'])) &&
        $module_config[$module_name]['activecomm'] == 0
    ) {
        $filled_field++;
    ...
    if (
        (empty($nv_laws_setting['detail_hide_empty_field']) or
            !empty($array_data['examine'])) &&
        $module_config[$module_name]['activecomm'] == 1
    ) {
        $filled_field++;
    ...
    if (nv_user_in_groups($array_data['groups_download'])) {
        if (!empty($array_data['files'])) {
            foreach ($array_data['files'] as $file) {
                ...
                if (
                    $file['ext'] == 'pdf' and
                    !empty($nv_laws_setting['detail_pdf_quick_view'])
                ) {
    ...
    if (!empty($other_subject)) {
        $xtpl->assign(
            'OTHER_SUBJECT',
            nv_theme_laws_list_other($other_subject)
        );
...
function nv_theme_laws_search($array_data, $generate_page, $all_page)
{
    ...
    $xtpl->assign(
        'NUMRESULT',
        sprintf($lang_module['s_result_num'], $all_page)
    );
...
function nv_theme_laws_list_other($array_data)
{
    ...
    foreach ($array_data as $row) {
        ...
        if (
            isset($site_mods['comment']) and
            !empty($module_config[$module_name]['activecomm'])
        ) {
...
function nv_theme_viewpdf($file_url)
{
    ...
    $xtpl = new XTemplate(
        'viewer.tpl',
        NV_ROOTDIR . '/' . NV_ASSETS_DIR . '/js/pdf.js'
    );
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('GLANG', $lang_global);
